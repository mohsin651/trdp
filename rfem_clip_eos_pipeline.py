"""
Modified RFEM on the CLIP TEXT encoder — EOS-row-calibrated threshold variant.

Sibling of rfem_clip_pipeline.py. The ONLY change is in Step 2:
per-head μ_h, σ_h used to compute the K-σ threshold are taken from the
EOS row of Â_h, not from the full lower triangle.

Why: under causal attention the row sizes vary — row i has (i+1) nonzero
entries summing to 1, so row-i mean = 1/(i+1). The full lower-triangle
mean is ≈ 2/(T+1), about 2× the EOS-row scale of 1/T. Threshold
μ + Kσ is therefore too high for the EOS row, and almost no EOS-row
entries survive (empty importance bars in the sibling file).

Fix: derive μ_h, σ_h from the row we read out (EOS). The mask itself is
still applied element-wise to the FULL T×T matrix, so all heatmap
diagrams stay full-shape — only the threshold value changes.

Everything else — Step 1 per-head rollout, Step 3 weighted aggregation
A_rfem = Σ w_h · Ā_h with w_h = max(Â_h), Step 4 EOS-row extraction,
value-preserving filter, plot styling, per-sentence subfolders, one PDF
per figure — is identical to rfem_clip_pipeline.py.

Note: w_h = max(Â_h) is still degenerate (= 1 for every head) because of
the BOS attention sink. That's a separate issue from the threshold; it
makes head-weighting effectively uniform but doesn't break the pipeline.
"""
...
OUT_DIR          = Path(__file__).resolve().parent / "figs_pdf_clip_eos"
...
def rfem_k_sigma_filter_causal(head_rollouts, k=0.5, eos_pos=None):
    """
    Value-preserving K-sigma filter — CAUSAL + EOS-row-calibrated variant.

    Per-head μ_h, σ_h are computed from Â_h[eos_pos, :] — the row we
    actually read out as the explanation — instead of from the full lower
    triangle. This calibrates the threshold to the same statistical scale
    as the EOS row.

        μ_h = mean(Â_h[eos_pos])
        σ_h = std(Â_h[eos_pos])

        Ā_h(i, j) = Â_h(i, j)  if  (i ≥ j)  AND  Â_h(i, j) ≥ μ_h + k * σ_h
                  = 0          otherwise

    The mask itself is still applied element-wise to the FULL T×T matrix
    (intersected with the lower triangle for causality), so heatmap shapes
    are unchanged.
    """
    if eos_pos is None:
        raise ValueError("eos_pos is required for EOS-calibrated filter")
    ...
    print(f"  μ, σ source: EOS row (position {eos_pos})")
    print(f"  {'Head':>5}  {'mu (EOS)':>13}  {'sigma (EOS)':>13}  {'threshold':>13}  {'kept':>14}")
    print(f"  {'-' * 68}")

    for h in range(H):
        R_h         = head_rollouts[h]
        eos_row     = R_h[eos_pos]               # (T,) — the row we extract
        mu_h        = eos_row.mean()
        sigma_h     = eos_row.std(unbiased=False)
...
def process_sentence(sid, text, label):
    ...
    for K in K_VALUES:
        ...
        head_masks, means, stds, thresholds, total_lower = \
            rfem_k_sigma_filter_causal(head_rollouts, k=K, eos_pos=eos_pos)
        ...
        plot_value_histogram(
            ...
            title=(f"{sid} Step 2 — Head {HEAD_TO_SHOW + 1} rollout distribution  (K={K})"
                   f"\n(lower triangle, <BOS>/<EOS> excluded;  μ,σ threshold from EOS row)"),
